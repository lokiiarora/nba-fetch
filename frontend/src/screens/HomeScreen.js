...
class HomeScreen extends React.Component {
  ...
  renderScoreBoardData = () => {
    const { data } = this.state.payload.scoreboard;
    const gridStyle = {
      width: "33%",
      textAlign: "center",
      padding: "10px"
    ...
    if(data.length === 0) {
      return (
        <Card className="no-data" title="Recent Games">
          No games today/tomorrow!
        </Card>
      );
    }
          ...
          return (
            ...
            >
              ...
              <Row className="meta-region">
                <p className="timing">
                  Starts at <TimeAgo>{new Date(item.startTimeUTC)}</TimeAgo>{" "}
                </p>
                  ...
                  {item.watch.broadcast.video ? (
                    item.watch.broadcast.video.deepLink ? (
                      item.watch.broadcast.video.deepLink[0] ? (
                        item.watch.broadcast.video.deepLink[0].desktopWeb ? (
                          <a
                            target="_blank"
