...
const fetchInitialPlayer = () => {
  axios
    .get("https://nbaproject.lokesh.co/api/v1/players")
    .then(res => {
      self.postMessage({
        payload: res.data,
        type: "success"
...
const fetchStandingsDataAndResolve = () => {
  Promise.all(fetchStandingsData())
    .then(values => {
      let obj = {
        div: null,
        conf: null
...
const fetchHomeData = () => {
  Promise.all([...fetchStandingsData(), fetchScoreBoardData()])
    .then(values => {
      let obj = {
        standings: {
          div: null,
...
const fetchScoreBoardData = () =>
  axios.get("https://nbaproject.lokesh.co/api/v1/scoreboard");

const fetchStandingsData = () => [
  axios.get("https://nbaproject.lokesh.co/api/v1/standings/div"),
  axios.get("https://nbaproject.lokesh.co/api/v1/standings/conf")
];
